...
async function sendDirectPushNotification(params) {
  ...
  try {
    ...
    // ✅ Build Android config - DATA-ONLY para que MyFirebaseMessagingService maneje todo
    // NO incluir android.notification para chat messages - causa que Android muestre
    // notificación vacía automáticamente sin pasar por el servicio nativo.
    // MyFirebaseMessagingService.kt lee las preferencias de SharedPreferences y selecciona el canal.
    const androidConfig = {
      priority: "high",
      // ✅ NO android.notification - el servicio nativo maneja canal/sonido/vibración
    };

    const message = {
      data: fcmData,
      tokens: fcmTokens,
      // ✅ NO incluir notification a nivel raíz - MyFirebaseMessagingService.kt maneja las notificaciones
      // con MessagingStyle y foto circular. Si se incluye notification, Android muestra automáticamente
      // sin pasar por el servicio nativo y no hay foto.
