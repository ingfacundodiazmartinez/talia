...
@objc class AppDelegate: FlutterAppDelegate, PKPushRegistryDelegate, CXProviderDelegate {
  ...
  override func userNotificationCenter(_ center: UNUserNotificationCenter,
    ...
    // StreamDetector ya maneja chats (~100ms) - FCM llega 2-5 segundos después
    ...
    // ✅ NUDGE: Suprimir notificación nativa pero reenviar datos a Flutter
    if notificationType == "nudge" {
      NSLog("📳 [Notifications] FOREGROUND - Nudge recibido, reenviando a Flutter")
      DispatchQueue.main.async {
        if let controller = self.window?.rootViewController as? FlutterViewController {
          let channel = FlutterMethodChannel(name: "com.talia.chat/nudge", binaryMessenger: controller.binaryMessenger)
          channel.invokeMethod("onNudgeReceived", arguments: userInfo)
          NSLog("📳 [Notifications] Nudge enviado a Flutter via method channel")
        }
      }
      completionHandler([])  // NO mostrar notificación nativa
      return
    }
