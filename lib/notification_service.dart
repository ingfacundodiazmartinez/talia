...
class NotificationService {
  ...
  Future<void> _initializeLocalNotifications() async {
    ...
    if (Platform.isAndroid) {
      ...
      // Canal para nudges con sonido personalizado
      const nudgeChannel = AndroidNotificationChannel(
        'nudge_channel',
        'Nudges',
        description: 'Canal para nudges recibidos',
        importance: Importance.high,
        enableVibration: true,
        playSound: true,
        sound: RawResourceAndroidNotificationSound('zumm'),
      );
      ...
      await plugin?.createNotificationChannel(nudgeChannel);
    }
  }
  ...
  Future<void> _setupListeners() async {
    ...
    _onMessageSub = FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      // 2. NUDGE: Mostrar overlay efímero
      if (messageType == 'nudge') {
        ReleaseLogger.log('📳 [Foreground] Nudge recibido - data: ${message.data}', tag: 'NotificationService');
        try {
          // Pasar datos al NudgeService para que maneje vibración + overlay
          NudgeService().handleIncomingNudge(message.data);
          ReleaseLogger.log('📳 [Foreground] NudgeService.handleIncomingNudge llamado', tag: 'NotificationService');
        } catch (e) {
          ReleaseLogger.error('❌ [Foreground] Error nudge: $e', tag: 'NotificationService');
        }
        return;
      }
      ...
      // 3. MENSAJES DE CHAT: StreamDetector + FCM fallback
      // ═══════════════════════════════════════════════════════════════
      // ARQUITECTURA CON FALLBACK:
      // 1. StreamDetector (Firestore listener) debería mostrar notificación ~100ms
      // 2. FCM llega después (~2-5s) y verifica si ya se mostró
      // 3. Si StreamDetector NO mostró notificación, FCM la muestra como fallback
      // ═══════════════════════════════════════════════════════════════
      if (messageType == 'chat_message' || messageType == 'group_message') {
        final messageId = message.data['messageId'] as String? ?? '';
        final chatId = message.data['chatId'] as String? ?? '';
        final isGroup = messageType == 'group_message';
        ...
        if (ChatStreamManager.wasNotificationShown(messageId)) {
          ReleaseLogger.log(
            '⏭️ [Foreground $platform] Notificación ya mostrada por StreamDetector - SKIP',
            tag: 'NotificationService',
          );
          return;
        }

        // ✅ FALLBACK: StreamDetector no mostró notificación, FCM la muestra
        ReleaseLogger.log(
          '🔔 [Foreground $platform] StreamDetector no mostró notificación - FCM fallback',
          tag: 'NotificationService',
        );

        // Marcar como mostrada para evitar duplicados
        ChatStreamManager.markNotificationShown(messageId);

        // ✅ CRÍTICO: Incrementar contador de no leídos (StreamDetector no lo hizo)
        if (chatId.isNotEmpty) {
          try {
            await LocalUnreadCountService().incrementUnreadCount(chatId);
            await UnreadMessagesService().updateBadgeCount();
            ReleaseLogger.log(
              '📊 [FCM Fallback] Unread count incrementado para chat $chatId',
              tag: 'NotificationService',
            );
          } catch (e) {
            ReleaseLogger.error(
              '❌ [FCM Fallback] Error incrementando unread: $e',
              tag: 'NotificationService',
            );
          }
        }

        // Obtener datos del mensaje para mostrar notificación
        final senderId = message.data['senderId'] as String? ?? '';
        final senderName = message.data['senderName'] as String? ?? 'Usuario';
        final body = message.data['body'] as String? ?? 'Mensaje nuevo';
        final senderPhotoUrl = message.data['senderPhotoUrl'] as String?;
        final groupName = isGroup ? (message.data['groupName'] as String?) : null;
        ...
        ReleaseLogger.log(
          '✅ [FCM Fallback] Notificación mostrada para mensaje $messageId',
          tag: 'NotificationService',
        );
        return;
      }
