...
class MessageStatusHelper {
  ...
  static MessageStatus calculateStatusV2({
    ...
    // Verificar si fue visto (mensaje antes o igual a lastOpenedAt del recipient)
    // ✅ FIX: Usar <= en lugar de < para evitar que mensajes con timestamps cercanos
    // queden sin marcar como leídos
    if (recipientLastOpenedAt != null) {
      ...
      // ✅ FIX: Usar <= para incluir mensajes con mismo timestamp
      final isBeforeOrEqual = msgMs <= openedMs;
    ...
    // Verificar si fue entregado (mensaje antes o igual a lastReceivedAt del recipient)
    if (recipientLastReceivedAt != null) {
      final msgMs = messageTimestamp.millisecondsSinceEpoch;
      final receivedMs = recipientLastReceivedAt.millisecondsSinceEpoch;
      if (msgMs <= receivedMs) {
  ...
  /// ✅ V2 ARCHITECTURE: Calcula MessageStatus para grupos usando lastOpenedAt
  ///
  /// Lógica para grupos (eficiente - O(1) por mensaje):
  /// - seen: TODOS los miembros abrieron el chat DESPUÉS del timestamp del mensaje
  /// - delivered: Al menos un miembro recibió el mensaje
  /// - sent: Tiene timestamp del servidor
  /// - sending: Aún no tiene timestamp
  ///
  /// [messageTimestamp] - Timestamp del mensaje
  /// [senderId] - ID del remitente del mensaje
  /// [otherMembers] - Lista de IDs de otros miembros (sin el sender)
  /// [membersLastOpenedAt] - Map de memberId -> lastOpenedAt de cada miembro
  /// [membersLastReceivedAt] - Map de memberId -> lastReceivedAt de cada miembro (opcional)
  static MessageStatus calculateGroupStatusWithTimestamps({
    required DateTime? messageTimestamp,
    required String senderId,
    required List<String> otherMembers,
    required Map<String, DateTime?> membersLastOpenedAt,
    Map<String, DateTime?>? membersLastReceivedAt,
  }) {
    final currentUserId = _auth.currentUser?.uid;
    if (currentUserId == null) return MessageStatus.sent;
    ...
    // Si no hay otros miembros, es "sent"
    ...
    // 1. Verificar si fue visto por TODOS los miembros
    // Un mensaje está "seen" si TODOS los otros miembros abrieron el chat después del mensaje
    final allSeen = otherMembers.every((memberId) {
      final memberLastOpened = membersLastOpenedAt[memberId];
      if (memberLastOpened == null) return false;
      return msgMs <= memberLastOpened.millisecondsSinceEpoch;
    });
    ...
    // 2. Verificar si fue entregado a TODOS los miembros
    if (membersLastReceivedAt != null) {
      final allDelivered = otherMembers.every((memberId) {
        final memberLastReceived = membersLastReceivedAt[memberId];
        if (memberLastReceived == null) return false;
        return msgMs <= memberLastReceived.millisecondsSinceEpoch;
      });
    ...
    // 3. Tiene timestamp = enviado
    return MessageStatus.sent;
  }

  /// ⚠️ LEGACY: Calcula MessageStatus para grupos usando readBy[]
  /// Usar calculateGroupStatusWithTimestamps para mejor rendimiento
  ///
  /// Lógica para grupos:
  ...
  /// - sent: Tiene timestamp del servidor
  /// - sending: Aún no tiene timestamp
  @Deprecated('Usar calculateGroupStatusWithTimestamps para mejor rendimiento')
